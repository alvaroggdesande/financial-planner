...
def run_scenario(config: ScenarioConfig) -> ScenarioConfig:
    ...
    for year in range(1, horizon + 1):
        ...
        yearly_results.append({
            'Year': year,
            'Net_Worth': net_worth_year,
            'Total_Assets': total_assets_year,
            'Assets_Cash': total_cash_year,
            'Assets_Stocks': total_stocks_year,
            'Assets_RealEstate_Equity': total_real_estate_equity_year,
            'Total_Liabilities': total_liabilities_year,
            'Liabilities_RealEstate_Debt': total_real_estate_debt_year,
            'Income_Sources_Total': total_income_from_sources_year,
            'RealEstate_Net_Cashflow': total_real_estate_cf_year, # What RE contributes/costs in cash
            'Annual_Living_Expenses_Est': annual_living_expenses_inflated,
            'Major_Expenses_Scheduled': current_year_major_expenses,
            'Net_Annual_Cash_Flow_Est': net_cash_flow_year
        })
    ...
    if not config.results_timeseries.empty:
        final_net_worth = config.results_timeseries['Net_Worth'].iloc[-1]
        # Add more like total contributions, total growth, average savings rate from scenario etc.
